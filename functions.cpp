...
float calculateHeadingAngle(float x, float y)
{
    ...
    float xDifference = x - currentX;
    float yDifference = y - currentY;
    float theta = atan(abs(xDifference/yDifference)) * RADIANS_TO_DEGREES;
...
double faceCoordinate(float x, float y) 
{
    float heading = calculateHeadingAngle(x, y);
    turnHeadingFast(heading);
    double newx = calculateXDistance(x);
    double newy = calculateYDistance(y);
...
void linefollowWhiteFast()
{
    ...
    if(MOpto.Value() <= BACKGROUND_OPTO_VALUE && LOpto.Value() <= BACKGROUND_OPTO_VALUE)
    {
        motorRight.SetPercent(45);
        motorLeft.SetPercent(30);
    }
    else if(MOpto.Value() <= BACKGROUND_OPTO_VALUE && ROpto.Value() <= BACKGROUND_OPTO_VALUE)
    {
        motorRight.SetPercent(30);
        motorLeft.SetPercent(45);
    }
    else if (LOpto.Value() <= BACKGROUND_OPTO_VALUE)
    {
        motorRight.SetPercent(30);
...
void leverSequence()
{
    ...
    driveDistance(1.5, 35);
    while(TimeNow() - endTime <= 5.1);
    servoCarrier.SetDegree(CARRIER_LEVER_UP_ANGLE);
    Sleep(250);
    servoCarrier.SetDegree(SET_TO_FLIP_LEVER_UP);
    driveDistance(.4, -35);
...
void passportSequence()
{
    ...
    Sleep(500);
    driveDistance(2.5,-LOW_SPEED_PCT);
    stopDrive();
...
void passportSequenceFastFast()
{

    int passportStartTime = TimeNow();
    while(TimeNow() - passportStartTime < 0.6)
    {
        motorLeft.SetPercent(-45);
        motorRight.SetPercent(-45);
    }
...
void RPStoLight(float x, float y)
{
    ...
    if(xDifference < 0)
    {
        theta = atan(abs(xDifference/yDifference)) * RADIANS_TO_DEGREES;
        theta = theta + 270;
    }
    else
    {
    ...
    turnHeadingFast(theta);

    while(CDSColor() != CDS_BLUE && CDSColor() != CDS_RED)
    {
        motorLeft.SetPercent(-35);
        motorRight.SetPercent(-35);
    }
...
void RPStoLightTO(float x, float y)
{
    ...
    while(CDSColor() != CDS_BLUE && CDSColor() != CDS_RED && TO == 0)
    {
        motorLeft.SetPercent(-35);
        motorRight.SetPercent(-35);

        if(TimeNow() - timeout >= 5)
        {
            stopDrive();
            driveDistance(4, 40);
            TO = 1;
...
void RPStoRamp(float x, float y)
{
    float currentDegree = RPS.Heading();
    float currentX = RPS.X();
    float currentY = RPS.Y();
    ...
    turnRightDegree(115);
    turnHeadingFastFast(theta);

    driveDistance(distance, -80);
...
void POI()
{
    ...
    LCD.WriteLine("SET LUGGAGE POINT");
    ...
    LCD.WriteLine("POINTS SET");
    Sleep(1.0);

    LCD.WriteLine("PRESS FOR NEXT");
